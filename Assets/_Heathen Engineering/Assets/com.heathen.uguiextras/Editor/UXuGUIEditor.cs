...
namespace HeathenEngineering.EditorScripts
{
    public class UXuGUIEditor
    {
        ...
        [MenuItem("Help/Heathen/UX uGUI/Update System Core (Package Manager)", priority = 1)]
        ...
        [MenuItem("Help/Heathen/UX uGUI/Documentation", priority = 2)]
        public static void Documentation()
        {
            Application.OpenURL("https://kb.heathenengineering.com/assets/physkit");
        }

        [MenuItem("Help/Heathen/UX uGUI/Support", priority = 3)]
        ...
        private static IEnumerator InitalizeAndLoad()
        {
            ...
            if (!SessionState.GetBool("UXuGUIInstallCheck", false))
            {
                SessionState.SetBool("UXuGUIInstallCheck", true);
#if !HE_SYSCORE
                if (EditorUtility.DisplayDialog("Heathen Installer", "System Core does not appear to be installed, and is a requirement for UX uGUI Extras to work properly. Would you like to install System Core?", "Install", "No"))
                {
                    yield return null;
